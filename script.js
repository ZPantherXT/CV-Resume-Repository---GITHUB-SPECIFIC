...
async function call(t){
    try {
        ...
        if(j.reply) document.getElementById('chat').innerHTML += `<div style="color:#FF8C00"><b>AI:</b> ${j.reply}</div>`;
...
function send(){ 
    ...
    if(i.value){ 
        document.getElementById('chat').innerHTML += `<div><b>You:</b> ${i.value}</div>`; 
        call(i.value); 
        i.value=''; 
    } 
}
